...
@IBDesignable open class CardHighlight: Card {
    ...
    override open func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        let buttonPoint = actionBtn.convert(point, from: self)
        if actionBtn.bounds.contains(buttonPoint) {
            return actionBtn.hitTest(buttonPoint, with: event)
        }
        return nil
    }
    
    public var viewButton: UIButton {
        return actionBtn
    }
}
